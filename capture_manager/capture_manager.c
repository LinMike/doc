...
int capmgr_mainloop()
{
    ...
    while (1)
    {
        ...
        for (;;)
        {
            ...
            {
                if (g_input_device->read_frame(&g_input_buffer.buf, &g_input_buffer.size))//从内存读取一帧数据
                {
                    if (process_frame(g_input_buffer.buf, g_input_buffer.size) < 0)
                    {
                        //log_error("process frame fail!\n");
...
int capmgr_open(capmgr_config_t* config)
{
    ...
    g_input_fd = g_input_device->open(&g_input_param);//g_input_device -> file、fpga、mock_contour8
    if (g_input_fd < 0) {
        log_error("failed to open input device!\n");
        goto ret_close;
    }
    //设置输出设备参数，输出到显示器hdmi，file yuv图片文件等
    ...
    g_bufsize = OUT_FRAME_SIZE;//一帧图片包含左右图片的Y和uv数据
    ...
    ret = g_input_device->start();//设备开启视频捕捉并把数据写入缓冲帧
    if (ret < 0) goto ret_close;

    if (g_capmgr_config.enable_preview) {
        ret = g_output_device->start();//开启视频输出设备
        if (ret < 0) goto ret_close;
    }

...
int main(int argc, char *argv[])
{
	...
	bgs_gmg bgs(cv::Size(IMAGE_WIDTH, IMAGE_HEIGHT), 1);
	std::vector<std::string> files;
	cv::glob(img_path, files);
//	for(std::vector<std::string>::iterator it = files.begin(); it != files.end(); it++)
	for(int k=0;k<files.size();k++)
	{
		std::cout<<"current process file name = "<<files[k]<<std::endl;
		cv::Mat img = cv::imread(files[k]), draw_img;
		draw_img=img.clone();
...
//		cv::dilate(fg_img, fg_img, elemente);
		if(k <= bgs.skip_frames) continue;

		std::cout<<"apply fg done"<<std::endl;
		std::vector<cv::KeyPoint> balls;
		bgs.DetectBall(fg_img.clone(), img, draw_img, balls);
		cv::namedWindow("fg img", CV_WINDOW_NORMAL);
//		cv::imshow("fg img", fg_img);
//		cv::waitKey(0);

		cv::Rect rect2;
		cv::namedWindow("img", CV_WINDOW_NORMAL);
		cv::setMouseCallback("img", on_mouse, (void*)&rect2);

		cv::imshow("fg img", fg_img);
		cv::imshow("img", draw_img);
		cv::waitKey(0);
		std::cout<<"mouse rectangle is "<<rect2<<std::endl;

		int roi_image_size = 30;
		cv::Rect rect(IMAGE_WIDTH/2-roi_image_size*5, 0, roi_image_size*10, IMAGE_HEIGHT);
		std::cout<<"set rectangle = "<<rect<<", balls size = "<<balls.size()<<std::endl;
		static int i=0;
		...
		{
			std::cout<<"ball pos = "<<it->pt.x<<", "<<it->pt.y<<std::endl;
			if(it->pt.x >= rect.tl().x && it->pt.x <= rect.br().x
					&& it->pt.y >= rect.tl().y && it->pt.y <= rect.br().y)
			{
				float lt_x = it->pt.x - roi_image_size <= IMAGE_WIDTH-1 ? (it->pt.x-roi_image_size > 0 ? it->pt.x-roi_image_size : 0) : IMAGE_WIDTH-1;
				float lt_y = it->pt.y - roi_image_size <= IMAGE_HEIGHT-1 ? (it->pt.y-roi_image_size > 0 ? it->pt.y-roi_image_size : 0) : IMAGE_HEIGHT-1;
				float rb_x = it->pt.x + roi_image_size <= IMAGE_WIDTH-1 ? (it->pt.x+roi_image_size > 0 ? it->pt.x+roi_image_size : 0) : IMAGE_WIDTH-1;
				float rb_y = it->pt.y + roi_image_size <= IMAGE_HEIGHT-1 ? (it->pt.y+roi_image_size > 0 ? it->pt.y+roi_image_size : 0) : IMAGE_HEIGHT-1;
				...
				cv::rectangle(draw_img, rect, cv::Scalar(0,0,255));
